...
classdef VehicleKinematicsEdge < g2o.core.BaseBinaryEdge
    ...
    methods(Access = public)
        ...
        function computeError(this)
            ...
            diff_x = this.edgeVertices{2}.x - priorX; % x_k+1 - x_k
            diff_x(3) = g2o.stuff.normalize_theta(diff_x(3)); % normaliza the phi part
            
            % compute error: inv(M) * (x_{k+1} - x_{k}) / dT - u
            this.errorZ = M_inv * diff_x / this.dT - this.z;
            % normalize the phi part
        ...
        function linearizeOplus(this)
            ...
            this.J{1}(1,1) = - c;
            this.J{1}(1,2) = - s;
            this.J{1}(1,3) = -diff_x(1) * s + diff_x(2) * c;
            this.J{1}(2,1) = s;
            this.J{1}(2,2) = - c;
            this.J{1}(2,3) = -diff_x(1) * c - diff_x(2) * s;
            this.J{1}(3,3) = -1;
            this.J{1} = this.J{1} / this.dT;

            % compute the Jacobian for x_{k+1}
            this.J{2} = M_inv / this.dT;
